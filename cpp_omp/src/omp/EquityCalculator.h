...
public:
    ...
    void setTimeLimit(double seconds)
    {
        ...
        mTimeLimit = seconds <= 0 ? INFINITE_CTM : seconds;
    }
    ...
    {
        std::lock_guard<std::mutex> lock(mMutex);
        mHandLimit = handLimit == 0 ? INFINITE_CTM : handLimit;
    }
...
private:
    ...
    static const uint64_t INFINITE_CTM = ~0ull;
    ...
    inline void randomizeBoard(Hand& board, unsigned remainingCards, uint64_t usedCardsMask,
                        Rng& rng, FastUniformIntDistribution<unsigned,16>& cardDist);
    template<bool tFlushPossible = true>
    inline void evaluateHands(const Hand* playerHands, unsigned nplayers, const Hand& board,
    ...
    HandEvaluator mEval;
    double mStdevTarget = 5e-5, mTimeLimit = (double)INFINITE_CTM, mUpdateInterval = 0.1;
    uint64_t mHandLimit = INFINITE_CTM;
